...
def fitness_function(matrix):
    ...
    try:
        output = subprocess.check_output(['java', '--add-opens', 'java.base/java.lang=ALL-UNNAMED', '-jar', 'SUB-main.jar'],text=True)
...
def polynomial_mutation(individual, mutation_rate, eta=20):
    ...
    for i in range(size[0]):
        if random.random() < mutation_rate:
            ...
            individual[i] += delta
            individual[i] = min(max(individual[i], -10), 10)
    return individual
...
def genetic_algorithm(population_size, generations):
    ...
    for generation in range(generations):
        ...
        for _ in range(population_size // 2):
            ...
            offspring1, offspring2 = crossover(parents, crossover_rate=0.2)
        ...
        allpopulation = population+new_population
        ...
        population = [allpopulation[i] for i in sorted_indices[:population_size]]
        #population = sortpopulation[:population_size]       
...
if __name__ == '__main__':
    best_solution,Fitness = genetic_algorithm(population_size=5, generations=5)
    print("Best solution:")
    print(best_solution)
